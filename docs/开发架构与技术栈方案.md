...
├── docs/
│   ├── Core Domain.md
│   ├── Task & Session 不可变规格.md
│   ├── 开发架构与技术栈方案.md
│   ├── 飞书-HITL-桥接最小API草案.md
│   └── 协作会话与工具会话集成-需求与功能设计汇总.md
├── apps/
│   ├── api/
│   │   ├── package.json
│   │   ├── tsconfig.json
...
│   │       │   ├── governance/
│   │       │   └── tooling/
│   │       │       ├── tool-session-link.service.ts
│   │       │       └── interaction-request.service.ts
│   │       ├── domain/
│   │       │   ├── common/
│   │       │   ├── project/
│   │       │   ├── session/
...
│   │       │   │           ├── tool-session-link.repository.ts
│   │       │   │           └── interaction-requests.repository.ts
│   │       │   ├── outbox/
│   │       │   ├── messaging/
│   │       │   ├── storage/
│   │       │   ├── plugins/
...
│       └── src/
│           ├── app/
│           │   ├── sessions/[collabSessionId]/page.tsx
│           │   ├── tools/[toolSessionId]/page.tsx
│           │   └── tasks/[taskId]/page.tsx
│           ├── components/
│           │   ├── collab-timeline.tsx
│           │   ├── tool-session-switcher.tsx
│           │   ├── tool-transcript-panel.tsx
│           │   └── summary-card.tsx
│           ├── lib/
│           │   ├── api-client.ts
│           │   └── query-client.ts
...
### 9.2 文件结构约束
- `apps/api/src/domain/` must not 依赖 `apps/api/src/infrastructure/`。
- `apps/api/src/application/` must 负责事务边界与命令编排，不承载领域规则本体。
- `apps/api/src/interfaces/` should only 做协议适配、鉴权、DTO 校验，不应出现业务决策。
- `apps/api/src/infrastructure/` must 封装外部系统（SQLite、本地分发、对象存储）细节。
- `apps/api/src/workers/` should 通过事件与命令协作，不应绕过应用层直接改聚合状态。
- `apps/api/src/infrastructure/plugins/` must 先验签后加载，未签名或验签失败插件 must not 执行。
- `apps/web/` must not 直接读写数据库，仅通过 `apps/api` 对外接口访问数据。
- `packages/contracts/` should 作为 API DTO 与共享类型唯一来源，避免前后端字段漂移。

### 9.3 v1 首批必须落地文件
- `apps/api/src/interfaces/http/dto/command-envelope.dto.ts`
- `apps/api/src/interfaces/http/dto/command-result.dto.ts`
- `apps/api/src/domain/common/error-codes.ts`
- `apps/api/src/infrastructure/persistence/sqlite/repositories/domain-events.repository.ts`
- `apps/api/src/infrastructure/persistence/sqlite/repositories/idempotency.repository.ts`
- `apps/api/src/infrastructure/persistence/sqlite/repositories/outbox.repository.ts`
- `apps/api/src/infrastructure/outbox/outbox.dispatcher.ts`
- `apps/api/src/workers/projection/projection.worker.ts`
- `apps/api/src/infrastructure/plugins/signature-verifier.ts`
- `apps/api/src/infrastructure/plugins/trust-store.repository.ts`
- `apps/api/src/interfaces/webhook/lark-interaction.webhook.ts`
- `apps/api/src/infrastructure/tool-runners/pty-run-manager.ts`
- `apps/api/src/infrastructure/persistence/sqlite/repositories/tool-session-link.repository.ts`
- `apps/api/src/infrastructure/persistence/sqlite/repositories/interaction-requests.repository.ts`
- `packages/contracts/src/hitl.ts`
