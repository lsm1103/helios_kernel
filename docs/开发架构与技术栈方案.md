...
4. **Infrastructure Layer（基础设施层）**
   - 持久化（状态库 + 事件库）、消息总线、对象存储、可观测性。
   - 提供 Outbox、审计日志、回放工具。

### 3.2 运行时组件
- `command-api`：命令入口与同步写入。
- `event-dispatcher`：消费 outbox，将领域事件发布到总线。
...
2. 发布到 Kafka/Redpanda 对应 topic。
3. 标记 outbox 已投递（至少一次语义）。
4. 下游 `projection-worker` 与策略服务订阅处理。
